...
public class DragAndDropFileTest {
  ...
  public void dragAndDropFile() {
    ...
    anAssert.assertEquals(textsOfElementsInTheFolder.size(), 1,
        "Quantity of elements in the folder differs from we've dropped");
    anAssert.assertEquals(textsOfElementsInTheFolder.get(0), FILE_NAME,
        "Name of the file differs from which we've dropped");
    anAssert.assertAll();
